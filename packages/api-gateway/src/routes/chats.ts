...
/**
 * Title is no longer accepted on create — chats start as "New chat"
 * and are renamed by the auto-summarizer (subtask 16656a) on the
 * first user message. Zod's default is non-strict so older clients
 * that still send `title` get it stripped silently rather than 400'd.
 */
...
chatsRouter.post("/", async (c) => {
  ...
  const [chat] = await db
    ...
    .values({
      id,
      tenantId,
      // `title` + `titleAutoGenerated` rely on the column defaults
      // ("New chat", true) — see subtask 16656a. Don't override here
      // or the summarizer's race-guard `WHERE` will skip the update.
  ...
  c.executionCtx.waitUntil(
    writeAudit(db, {
      ...
      action: "chat.create",
      target: id,
      payload: { dbProfileId: body.dbProfileId ?? null },
    })
  );
...
chatsRouter.patch("/:id", async (c) => {
  ...
  if (body.title !== undefined) {
    ...
    // A manual rename means the user owns the title from now on; the
    // auto-summarizer's race-guard `WHERE title_auto_generated = true`
    // (subtask 16656a) ensures it backs off after this point.
    patch.titleAutoGenerated = false;
  }
